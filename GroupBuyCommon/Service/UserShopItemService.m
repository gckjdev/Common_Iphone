...
- (void)requestItemMatchCount:(NSString*)itemId tableViewController:(ShoppingListController*)shoppingListController
{
...
- (void)deleteUserShoppingItem:(NSString*)itemId
{
    ...
    NSString* appId = [AppManager getPlaceAppId];
    if ([self.userShopItemServiceDelegate respondsToSelector:@selector(didBeginUpdateShoppingItem:)]) {
        [self.userShopItemServiceDelegate didBeginUpdateShoppingItem:@"删除数据中......"];
    }
...
- (void)updateUserShoppingItemCountList:(PPTableViewController *)tableViewController
{
    ...
    [self refreshUserShoppingItemMatchCount:nil appId:appId userId:userId];
